...
wallet_address = web3.to_checksum_address(config['wallet_address'])  # metamask wallet address
private_key = config['private_key']  #. metamask wallet private key
token_to_buy = web3.to_checksum_address(config['buy_token'])  # The token that you want to buy (e.g. BUSD )
wbnb = web3.to_checksum_address(config['wbnb'])  # The token that you spend (It's constant)
# uniswap factory address and abi = pancakeswap factory 0x5C69bEe701ef814a2B6a3EDD4B1652CB9cc5aA6f 
uniswap_factory = web3.to_checksum_address('0xcA143Ce32Fe78f1f7019d7d551a6402fC5350c73')  #Testnet  #0x6725F303b657a9451d8BA641348b6761A6CC7a17
...
try:
    ...
    print(e)
    exit()
print(f"Balance of {wallet_address}: {web3.from_wei(web3.eth.get_balance(wallet_address), 'ether')}")
...
def buy(intoken, outtoken):
    ...
    #     ).buildTransaction({
    #         'from': sender_address,
    #         'value': web3.to_wei(config['amount'], 'ether'),
...
def approve():
    try:
        ...
        txn.update({'gas': int(estimate_gas(txn))})
...
def sell(intoken, outtoken):
    ...
    #         'value': 0,
    #     })
    #     pancakeswap2_txn.update({'gas': int(estimate_gas(pancakeswap2_txn))})
    
    #     print(pancakeswap2_txn)
    #     signed_txn = web3.eth.account.sign_transaction(pancakeswap2_txn, private_key=config['private_key'])
    #     print(signed_txn)
    #     tx_token_sell = web3.eth.send_raw_transaction(signed_txn.rawTransaction)
...
def check_pool(inToken, outToken):
    ...
    pair_address = factoryContract.functions.getPair(outToken, inToken).call()
...
def reach_profit(intoken, outtoken):
    try:
        ...
        print(f"Current price: {format(current_price, '.4f')}")
        print(f"Expected price: {format(expected_price, '.4f')}")
        print(f"Profit percentage: {format((current_price/float(buy_price) - 1)*100, '.2f')}%")
...
if __name__=="__main__":
    interrect_token = web3.to_checksum_address(config['buy_token'])
    print("No way", check_pair(wbnb, interrect_token))



    if fetch_liquidity(wbnb, interrect_token):
        buy_price = get_token_price(wbnb, interrect_token)
        print("Pair already exists")
        print(buy_price)
        expected_price = float(buy_price) * (1 + float(config['takeprofit'])/100)
        print(f"E {expected_price}")
        main(wbnb, interrect_token)
    else:
        print("Pair not found")
        exit()
    # reach_profit(wbnb, web3.to_checksum_address('0xf20702BD4D8D1DDbe3c9a64fdb9E8132a4B1839D'))
    # fetch_liquidity(wbnb, web3.to_checksum_address('0xF952Fc3ca7325Cc27D15885d37117676d25BfdA6'))
    ...
    # get_token_price(web3.to_checksum_address('0xf952fc3ca7325cc27d15885d37117676d25bfda6'))
    # print(check_pool(wbnb, web3.to_checksum_address('0xF952Fc3ca7325Cc27D15885d37117676d25BfdA6')))
    # sell(wbnb, web3.to_checksum_address('0xe9e7cea3dedca5984780bafc599bd69add087d56'))
    end = datetime.now()
